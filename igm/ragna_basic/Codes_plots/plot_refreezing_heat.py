...
# ----------------------------
date_simu = "2026-03-10/12-12-57/"


simu_path = os.path.join( "../outputs", date_simu)
out_dir = os.path.join("../outputs", date_simu, "Plots/refreezing")
...
print(" ******** REFREEZING HEAT ******** ")
print("          Mean : ", np.mean(T_var.values))
print("          Min : ", np.nanmin(T_var.values))
print("          Max : ", np.nanmax(T_var.values))
...
print("Interpolation de refreezing heat complétée.")
print("refreezing_along_flow shape:", T3d_along_flow.shape)
...
print("Moyenne verticale pondérée de refreezing heat complétée.")
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(out_dir, f"refreezing_heat_profiles_along_{which_flowline}_flowline.png"), dpi=200)
plt.show()
...
plt.title(f"Vertical distribution of refreezing heat component – {which_flowline} flowline, {year_i},final State")
plt.legend()
plt.grid(True, linestyle=":")
plt.tight_layout()
plt.savefig(os.path.join(out_dir, f"refreezing_heat_vertical_section_altitude_{which_flowline}_{years[i_last]}_Etat_final.png"), dpi=200)
plt.show()
...
for i_last in indices:
    ...
    plt.xlabel("Distance along the flowline (km)")
    plt.ylabel("Altitude (m a.s.l.)")
    plt.title(f"Vertical distribution of refreezing heat component – {which_flowline} flowline, {year_i}")
    plt.legend()
    plt.grid(True, linestyle=":")
    plt.tight_layout()
    ...
    if MAKE_GIF == False and (i_last == 0 or i_last == len(time)-1) :
        fname = os.path.join(out_dir, f"refreezing_heat_vertical_section_altitude_{which_flowline}_{year_i}.png")
...
# ================================
if MAKE_GIF:
    gif_path = os.path.join(out_dir, f"refreezing_heat_vertical_section_altitude_{which_flowline}_ANIMATION.gif")
