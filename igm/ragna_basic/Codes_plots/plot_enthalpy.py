...
# ----------------------------
date_simu = "2026-03-09/09-04-25/"


simu_path = os.path.join( "../outputs", date_simu)
out_dir = os.path.join("../outputs", date_simu, "Plots/enthalpy")
...
MAKE_GIF = False     # ← Mets False si tu veux juste le dernier plot
...
plt.ylabel("Mean enthalpy J kg−1")
plt.title(f"Evolution of mean enthalpy along the flowline {which_flowline}")
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(out_dir, f"E_profiles_along_{which_flowline}_flowline.png"), dpi=200)
plt.show()
...
cont = plt.contourf(X, Z_abs, T3d_crop, levels=levels, cmap="turbo") #vmin = 270, vmax=273)
cbar = plt.colorbar(cont, label="Enthalpy J kg−1")
