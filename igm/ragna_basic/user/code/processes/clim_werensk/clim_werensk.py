...
def  initialize(cfg,state):
    """
        load climate data to run the Werenskioldbreen simulation
    """
    ...
    temp_prec = np.loadtxt(
        state.original_cwd.joinpath(os.path.join("data", "clim_ragna_1750_prec.dat")),
        dtype=np.float32,
        skiprows=1,
    )
    ...
    if cfg.processes.clim_werensk.time_resolution==12:
    ...
    if "time" not in cfg.processes:
        raise ValueError("The 'time' module is required for the 'clim_werensk' module.")

    state.tlast_clim_werensk = tf.Variable(cfg.processes.time.start)
    state.tcomp_clim_werensk = []


def update(cfg,state):

    if ((state.t - state.tlast_clim_werensk) >= cfg.processes.clim_werensk.update_freq):

        if hasattr(state, "logger"):
            state.logger.info("update climate at time : " + str(state.t.numpy()))

        state.tcomp_clim_werensk.append(time.time())
        ...
        with tf.device('/CPU:0'):
            new_prec = tf.clip_by_value(state.precipitation * prec_corr_mult, 0, 10 ** 10)
        ...
        state.tlast_clim_werensk.assign(state.t)

        state.tcomp_clim_werensk[-1] -= time.time()
        state.tcomp_clim_werensk[-1] *= -1
