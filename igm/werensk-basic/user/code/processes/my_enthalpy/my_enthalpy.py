...
def initialize(cfg, state):
    ...
    if "iceflow" not in cfg.processes:
        raise ValueError("The 'iceflow' module is required for the 'my_enthalpy' module.")
    
    Ny, Nx = state.thk.shape
    Nz = cfg.processes.iceflow.numerics.Nz

    # ---------- LOAD INPUT TEMPERATURE IF PROVIDED (if not, keep old structure input_temperature_file="None") ----------
    if hasattr(cfg.processes.my_enthalpy, "input_temperature_file") \
       and cfg.processes.my_enthalpy.input_temperature_file not in [None, "", "None"]:

        ds = xr.open_dataset(cfg.processes.my_enthalpy.input_temperature_file) 
        Tin = ds[cfg.processes.my_enthalpy.input_temperature_variable] # input T
        print( "---------------- TEMPERATURE INPUT DOWNLOADED ----------------------" )
        # Convert to numpy
        Tnp = Tin.values
        print( "----- INPUT ----- Tmin = ", np.min(Tnp), ", Tmax = ", np.max(Tnp), "----------------" )

        # Detect dimensions
        if Tnp.ndim == 3:
            print( "---------------- TEMPERATURE INPUT 3D ----------------------" )
            Nz_in, Ny_in, Nx_in = Tnp.shape
        elif Tnp.ndim == 2:
            Ny_in, Nx_in = Tnp.shape # in case only 2D field available, this choice is questionable, remplacer à terme par  
            Nz_in = 1
            Tnp = Tnp[None, :, :]  # add vertical dimension
        else:
            raise ValueError("Temperature in netCDF must be 2D or 3D.")

        # Build vertical coordinate for input (assume evenly spaced if absent)
        if "z" in Tin.dims:
            z_in = Tin["z"].values
            print(" dim de z input", z_in)
        else:
            z_in = np.linspace(0, 1, Nz_in)

        # Build IGM vertical coordinate
        z_igm = compute_levels(cfg.processes.iceflow.numerics.Nz, cfg.processes.iceflow.numerics.vert_spacing)

        # ---------- VERTICAL INTERPOLATION ----------
        # shape: (Nz, Ny_in, Nx_in)
        T_vert_interp = np.zeros((Nz, Ny_in, Nx_in))
        for j in range(Ny_in):
            for i in range(Nx_in):
                T_vert_interp[:, j, i] = np.interp(z_igm, z_in, Tnp[:, j, i])

        # ---------- HORIZONTAL INTERPOLATION ----------
        # New (Ny, Nx) grid
        y_out = np.linspace(0, 1, Ny)
        x_out = np.linspace(0, 1, Nx)
        y_in  = np.linspace(0, 1, Ny_in)
        x_in  = np.linspace(0, 1, Nx_in)

        # 2D interpolation for each vertical level
        T_final = np.zeros((Nz, Ny, Nx))
        for k in range(Nz):

    
            interp2d = RegularGridInterpolator(
                 (y_in, x_in),          # axes d’entrée : (Ny, Nx)
                 T_vert_interp[k],      # champ à interpoler (Ny, Nx)
                 method="linear",
                 bounds_error=False,
                 fill_value=np.nan
            )

            # Construction des points (y,x) -> shape (Ny_out*Nx_out, 2)
            yy, xx = np.meshgrid(y_out, x_out, indexing='ij')
            points = np.column_stack((yy.ravel(), xx.ravel()))

    
            T_final[k] = interp2d(points).reshape(Ny, Nx)
            T_final = T_final.astype(np.float32)
            print( "----- INTERPOLATED LEVEL ",k, " ----- Tmin = ", np.min(T_final), ", Tmax = ", np.max(T_final), "----------------" )
        # Convert to TF variable
        state.T = tf.Variable(T_final, trainable=False)

    else:
        # ----------- DEFAULT CONSTANT TEMPERATURE -----------
        state.T = tf.Variable(
            tf.ones((Nz, Ny, Nx)) * cfg.processes.my_enthalpy.melt_temp,
            trainable=False
        )
...
########## REMAINS OFF INITIAL ENTHALPY MODULE ##########

    state.basalMeltRate = tf.Variable(tf.zeros_like(state.thk), trainable=False)
    #state.T = tf.Variable(tf.ones((cfg.processes.iceflow.numerics.Nz, Ny, Nx)) * cfg.processes.enthalpy.melt_temp, trainable=False)
    state.omega = tf.Variable(tf.zeros_like(state.T), trainable=False)
    state.E = tf.Variable(
        tf.ones_like(state.T)
        * (cfg.processes.my_enthalpy.ci * (state.T - cfg.processes.my_enthalpy.ref_temp)),
        trainable=False,
    ) ### E initialized like if ice were cold
    ...
    if not hasattr(state, "bheatflx"):
        state.bheatflx = tf.Variable(
            tf.ones_like(state.thk) * cfg.processes.my_enthalpy.default_bheatflx, trainable=False
        )
    ...
    state.tauc, state.slidingco = compute_slidingco_tf(
        ...
        cfg.processes.my_enthalpy.till_wat_max,
        state.phi,
        cfg.processes.iceflow.physics.exp_weertman,
        cfg.processes.my_enthalpy.uthreshold, 
        cfg.processes.my_enthalpy.tauc_min,
        cfg.processes.my_enthalpy.tauc_max,
    )
...
def update(cfg, state):
    ...
    surftemp = (
        tf.minimum(
            tf.math.reduce_mean(
                state.air_temp + cfg.processes.my_enthalpy.temperature_offset_air_to_ice, axis=0
            ),
            0,
        )
        + cfg.processes.my_enthalpy.melt_temp
    )  # [K]
    ...
    Tpmp, Epmp = TpmpEpmp_from_depth_tf(
        ...
        cfg.processes.iceflow.physics.gravity_cst,
        cfg.processes.iceflow.physics.ice_density,
        cfg.processes.my_enthalpy.claus_clape,
        cfg.processes.my_enthalpy.melt_temp,
        cfg.processes.my_enthalpy.ci,
        cfg.processes.my_enthalpy.ref_temp,
    )
    ...
    state.strainheat = compute_strainheat_tf(
        state.U / cfg.processes.my_enthalpy.spy,
        state.V / cfg.processes.my_enthalpy.spy,
        state.arrhenius,
        state.dx,
        dz,
    ...
    surfenth = surf_enthalpy_from_temperature_tf(
        surftemp, cfg.processes.my_enthalpy.melt_temp, cfg.processes.my_enthalpy.ci, cfg.processes.my_enthalpy.ref_temp
    )
    ...
    state.E, state.basalMeltRate = compute_enthalpy_basalmeltrate(
        state.E,
        Epmp,
        state.dt * cfg.processes.my_enthalpy.spy,
        dz,
        Wc / cfg.processes.my_enthalpy.spy,
        surfenth,
        state.bheatflx,
        state.strainheat,
        state.frictheat,
        state.tillwat,
        cfg.processes.iceflow.physics.thr_ice_thk,
        cfg.processes.my_enthalpy.ki,
        cfg.processes.iceflow.physics.ice_density,
        cfg.processes.my_enthalpy.water_density,
        cfg.processes.my_enthalpy.ci,
        cfg.processes.my_enthalpy.ref_temp,
        cfg.processes.my_enthalpy.Lh,
        cfg.processes.my_enthalpy.spy,
        cfg.processes.my_enthalpy.KtdivKc,
        cfg.processes.my_enthalpy.drain_ice_column,
    )
    ...
    state.tillwat = state.tillwat + state.dt * (
        state.basalMeltRate - cfg.processes.my_enthalpy.drain_rate
    )
    state.tillwat = tf.clip_by_value(state.tillwat, 0.0, cfg.processes.my_enthalpy.till_wat_max)
...
def compute_phi(cfg, state):
    ...
    if cfg.processes.my_enthalpy.till_friction_angle_bed_min == None:
        return cfg.processes.my_enthalpy.till_friction_angle * tf.ones_like(state.thk)
    else:
        return tf.where(
            state.topg <= cfg.processes.my_enthalpy.till_friction_angle_bed_min,
            cfg.processes.my_enthalpy.till_friction_angle_phi_min,
            tf.where(
                state.topg >= cfg.processes.my_enthalpy.till_friction_angle_bed_max,
                cfg.processes.my_enthalpy.till_friction_angle_phi_max,
                cfg.processes.my_enthalpy.till_friction_angle_phi_min
                + (
                    cfg.processes.my_enthalpy.till_friction_angle_phi_max
                    - cfg.processes.my_enthalpy.till_friction_angle_phi_min
                ...
                * (state.topg - cfg.processes.my_enthalpy.till_friction_angle_bed_min)
                / (
                    cfg.processes.my_enthalpy.till_friction_angle_bed_max
                    - cfg.processes.my_enthalpy.till_friction_angle_bed_min
                ),
            ),
        )
