...
elif glacier_name == "ragna_mariebreen":
    x_min, x_max, y_min, y_max= [552400, 558500, 8632800, 8640500]
...
biharmonic=False
sigma = 2.5 #gaussian filter smoothing the higher, the smoother
#sigma = 3.5 ## better for kropp that is a noisy boy
...
for csv_file in csv_files:
    ...
    h_trace = gpr_df_trace["thickness"]
...
h = pd.concat(h).values
surf = pd.concat(surf).values


print("thk between ", np.min(h), " & ", np.max(h))
...
Hi = gaussian_filter(Hi, sigma=sigma)

mask = np.zeros(Hi.shape, dtype=bool)
...
Hi_masked = np.ma.array(Hi, mask=mask)
Surfi_masked = np.ma.array(Surfi, mask=mask)

plt.scatter(Xi, Yi,c=Hi_masked, s=3)
plt.xlabel("x")
plt.ylabel("y")
plt.colorbar(label="Thickness")
plt.show()
...
plt.pcolormesh(
    Xi, Yi, Surfi_masked-Hi_masked,
    shading="auto", cmap="terrain" 
)

plt.colorbar(label="Bed elevation")
plt.title("Bedrock")
plt.xlabel("X")
plt.ylabel("Y")
...
#H_filled = np.flipud(Hi_masked)
#Surf_filled = np.flipud(Surfi_masked)

H_filled = Hi_masked
Surf_filled = Surfi_masked

Z_filled = Surf_filled - H_filled
