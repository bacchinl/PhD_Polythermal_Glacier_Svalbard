...
year_surf = 1990
nom_glacier = "S-Heerland" #"Ragna-Mariebreen" ou "Mettebreen"

if nom_glacier == "NW-Heerland" :
    x_min, x_max, y_min, y_max= [543500, 566500, 8633000, 8655000]

    glaciers_to_exclude = ["Paulabreen","Nordsysselbreen", "Bakaninbreen", "Inglefieldbreen", "Sørbullbreen", "Schleelebreen", "Helsingborgbreen", "Peisbreen", "Andrinebreen"] ## I want the littles snowpatchs but no big tide water glaciers in this simu and border ones

elif nom_glacier == "NE-Heerland" :
    x_min, x_max, y_min, y_max= [563000, 579000, 8650000, 8666000]

    glaciers_to_exclude = ["Edvardbreen", "Nordsysselbreen"]

elif nom_glacier == "S-Heerland" :
    x_min, x_max, y_min, y_max= [549000, 581500, 8603000, 8651500]

    glaciers_to_exclude = ["Edvardbreen", "Ragna-Maribreen", "Mettebreen","Kroppbreen","Vallåkrabreen", "Doktorbreen", "Perseibreen", "Hyllingebreen", "Bjuvbreen", "Billesholmbreen", "Bosarpbreen", "Liestølbreen", "Scheelebreen", "Sørbullbreen", "Andrinebreen", "Andrinebreen S"]
...
#data_path = "BEDS_PROCESSED" ## For corrected beds
data_path = "../Datasets"

#ds_bed = xr.open_dataset(os.path.join(data_path, f"Bedrock_{nom_glacier}_{src_bed}_res_50m.nc"))
ds_bed = xr.open_dataset(os.path.join(data_path, f"Bedrock_{src_bed}_HEERLAND_50m.nc"))
...
output_path = os.path.join(dir_output, f"input_{nom_glacier}_{year_surf}_{src_bed}_bed_res{res_target}m.nc")
