...
#nom_glacier = "Kroppbreen" #"Ragna-Mariebreen" ou "Mettebreen"
...
if source_bed == "GPR":
    data_path = "."
    ds_bed = xr.open_dataset(os.path.join(data_path, f"Bedrock_global_50m.nc"))

    bed = ds_bed['topg']

    x_bed = ds_bed['x'].values
    y_bed = ds_bed['y'].values
    print("    ",x_bed[:5], x_bed[-5:])
    print("    ",y_bed[:5], y_bed[-5:])

    if y_bed[0] > y_bed[-1]:
        yslice = slice(y_max, y_min)   # axe décroissant
    else:
        yslice = slice(y_min, y_max)   # axe croissant

    bed_sub = bed.sel(x=slice(x_min, x_max),y=yslice)
...
if nom_glacier == "Global" :
    glaciers = ["Mettebreen", "Ragna-Mariebreen", "Vallåkrabreen", "Kroppbreen", "Edvardbreen"]# only data for these five
...
## cut of pixels that could create discontinuity 
#bed_gpr_cut = np.where(mask_tot_ice == 0, np.nan, bed_array)
n_pixels = 2      # 2 pixels de 50 m = 100 m

mask_inner = binary_erosion(mask_tot_ice.astype(bool),
                            iterations=n_pixels)

bed_gpr_cut = np.where(mask_inner, bed_array, np.nan)
...
dir_output = "."
output_path = os.path.join(dir_output, f"Bedrock_{nom_glacier}_{source_bed}_and_vp_res_{res}m.nc")
