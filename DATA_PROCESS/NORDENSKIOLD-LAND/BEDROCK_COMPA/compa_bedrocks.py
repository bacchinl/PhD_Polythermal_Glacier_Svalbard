...
# GLOBAL PARAMETERS
# =========================================================
xmin, xmax, ymin, ymax = [492500, 555500,  8597500, 8637500]
resolution = 50
plot_results = False
...
def load_and_crop_tif_new(path, xmin, xmax, ymin, ymax):
...
def plot_beds_side_by_side(beds, names):
    fig, axes = plt.subplots(1, len(names), figsize=(len(names)*6+1, 6))
...
# =========================================================
region_path = "~/PhD_Lucie/DATA/Maps/Shapefiles_regions/mask_nordenskioldland.shp"
...
# Convert to numpy
bed_furst_array = bed_furst.values.astype(np.float32)
...
# 4. VAN PELT BEDROCK
# =========================================================
vp_path = os.path.expanduser("~/PhD_Lucie/DATA/BEDROCK/Van_pelt/Bed_map.tif")
...
# 5. FRANCK BEDROCK
# =========================================================
franck_path = "mosaic_nordenskiold_land_franck.tif"
...
path_millan =  os.path.expanduser("~/PhD_Lucie/DATA/THICKNESS/MILLAN_RGI7/RGI-7/THICKNESS_RGI-7.1_2021July09.tif")
thk_millan, millan_transform, millan_crs = load_and_crop_tif_new(path_millan, xmin, xmax, ymin, ymax)

thk_millan_grid = reproject_raster_to_grid(
    thk_millan, millan_transform, millan_crs,
    grid_transform,
    grid_shape
)
...
path_npi =  os.path.expanduser("~/PhD_Lucie/DATA/DEMS/DEM-2010-NPI/DEM20m_svalbard_esa_v2.tif")
usurf_npi, npi_transform, npi_crs = load_and_crop_tif_new(path_npi, xmin, xmax, ymin, ymax)
usurf_npi_grid = reproject_raster_to_grid(
    usurf_npi, npi_transform, npi_crs,
    grid_transform,
    grid_shape
)
...
bed_millan_grid = usurf_npi_grid - thk_millan_grid 
plot_beds(bed_millan_grid, "Millan")
...
bed_millan_masked = np.where(glacier_mask == 0, np.nan, bed_millan_grid)
...
if plot_results :
    ...
    plot_beds(bed_millan_masked, "Millan")

plot_beds_side_by_side(
    [bed_furst_masked, bed_vp_masked, bed_franck_masked, bed_millan_masked],
    ["Fürst", "Van Pelt", "Franck","Millan"]
)
...
def save_bedrock_to_netcdf(bedrock_array,
    ...
    output_path = os.path.join(
        output_dir,
        f"Bedrock_{name}_HEERLAND_{resolution}m.nc"
    )
