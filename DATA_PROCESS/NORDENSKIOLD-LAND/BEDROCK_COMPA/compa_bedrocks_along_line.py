...
data_dir = "~/PhD_Lucie/DATA/GPR/Mannerfelt/thickness_cts_points_csv"

radar_line = "vallakrabreen-20210513-DAT_0012_A1_1"
glacier_name = "vallakrabreen"
...
mean_error_furst = np.nanmean(bed_furst_along_line-bed_gpr_obs)
mean_error_vp = np.mean(bed_vp_along_line-bed_gpr_obs)
mean_error_franck = np.mean(bed_franck_along_line-bed_gpr_obs)

std_furst = np.nanstd(bed_furst_along_line-bed_gpr_obs)
std_vp = np.std(bed_vp_along_line-bed_gpr_obs)
std_franck = np.std(bed_franck_along_line-bed_gpr_obs)

print("ERROR FURST ", mean_error_furst, "m")
print("ERROR VanPelt ", mean_error_vp, "m")
print("ERROR FRANCK ", mean_error_franck, "m")

print("STD FURST ", std_furst, "m")
print("STD VanPelt ", std_vp, "m")
print("STD FRANCK ", std_franck, "m")
...
plt.plot(
    ...
    label=f"Furst, mean error : {mean_error_furst}m, std {std_furst}"
)

plt.plot(
    distance,
    bed_vp_along_line,
    label=f"Van Pelt, mean error : {mean_error_vp}m, std {std_vp}"
)

plt.plot(
    distance,
    bed_franck_along_line,
    label=f"Franck, mean error : {mean_error_franck}m, std {std_franck}"
)
...
plt.savefig(f"Plots/{glacier_name}/Bedrock_comparaison_{radar_line}.jpg")
plt.show()
