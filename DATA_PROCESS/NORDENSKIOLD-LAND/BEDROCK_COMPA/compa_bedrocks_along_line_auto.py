...
furst_nc_path = os.path.join(
    path_bedrocks,
    f"Bedrock_Furst_{region}_{resolution}m.nc"
)

vp_nc_path = os.path.join(
    path_bedrocks,
    f"Bedrock_VanPelt_{region}_{resolution}m.nc"
)

franck_nc_path = os.path.join(
    path_bedrocks,
    f"Bedrock_Franck_{region}_{resolution}m.nc"
)

millan_nc_path = os.path.join(
    path_bedrocks,
    f"Bedrock_Millan_{region}_{resolution}m.nc"
)
...
ds_millan = xr.open_dataset(millan_nc_path)
...
bed_millan = ds_millan["topg"].values
...
interp_millan = RegularGridInterpolator(
    (y, x),
    bed_millan,
    bounds_error=False,
    fill_value=np.nan
...
for csv_file in csv_files:
    ...
    bed_millan_along_line = interp_millan(points_gpr)
    ...
    plt.plot(
        distance,
        bed_millan_along_line,
        label=(
            f"Milan "
            f"(bias={mean_error_millan:.2f} m, "
            f"std={std_millan:.2f} m)"
        )
    )
